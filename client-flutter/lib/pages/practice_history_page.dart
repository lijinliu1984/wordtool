...
class PracticeHistoryPage extends StatefulWidget {
  ...
  const PracticeHistoryPage({super.key, this.date});
...
class _PracticeHistoryPageState extends State<PracticeHistoryPage> {
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: widget.date != null
              ? Text('练习记录 · ${widget.date!.month}月${widget.date!.day}日')
              : null,
          bottom: const TabBar(
            tabs: [
  ...
  Widget _buildList(String type) {
    final future = type == 'listening'
        ? UserDatabase.instance.getPracticeResultsByTypes([
            'listening',
            'listening_image',
            'listening_text',
          ], date: widget.date)
        : UserDatabase.instance.getPracticeResults(type, date: widget.date);
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final taskName = record['task_name'] as String?;
